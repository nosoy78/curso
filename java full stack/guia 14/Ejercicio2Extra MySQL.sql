...
SELECT c.nombre_cliente, e.nombre 
AS nombre_representante, o.ciudad 
AS ciudad_oficina 
FROM cliente c 
INNER JOIN empleado e 
ON c.codigo_empleado_rep_ventas = e.codigo_empleado 
...
SELECT o.linea_direccion1, o.linea_direccion2, o.ciudad, o.pais 
FROM oficina o 
WHERE o.codigo_oficina 
IN (SELECT e.codigo_oficina 
FROM empleado e 
...
from cliente c 
inner join empleado e 
on e.codigo_empleado = e.codigo_empleado 
inner join oficina o 
...
SELECT c.codigo_cliente, c.nombre_cliente, GROUP_CONCAT(DISTINCT p.gama SEPARATOR ', ') 
AS gamas_compradas 
FROM cliente c 
INNER JOIN pedido pd 
ON c.codigo_cliente = pd.codigo_cliente 
INNER JOIN detalle_pedido dp 
ON pd.codigo_pedido = dp.codigo_pedido 
...
SELECT c.codigo_cliente, c.nombre_cliente, 'Sin pago' 
AS tipo 
FROM cliente c 
LEFT JOIN pago p 
ON c.codigo_cliente = p.codigo_cliente 
WHERE p.codigo_cliente 
IS NULL UNION SELECT c.codigo_cliente, c.nombre_cliente, 'Sin pedido' 
AS tipo 
FROM cliente c 
LEFT JOIN pedido pd 
ON c.codigo_cliente = pd.codigo_cliente 
...
ON e.codigo_empleado = c.codigo_empleado_rep_ventas 
WHERE o.codigo_oficina 
IS NULL AND c.codigo_empleado_rep_ventas IS NULL;
...
AS precio_mas_caro, MIN(precio_venta) 
...
SELECT COUNT(*) 
AS cantidad_clientes_sin_representante 
FROM cliente 
WHERE codigo_empleado_rep_ventas IS NULL;

-- 11. Calcula la fecha del primer y último pago realizado por cada uno de los clientes. El listado deberá mostrar el nombre y los apellidos de cada cliente

SELECT c.nombre_cliente, c.nombre_contacto, c.apellido_contacto, MIN(pa.fecha_pago) 
AS primera_fecha_pago, MAX(pa.fecha_pago) 
...
AS cantidad_productos_diferentes 
FROM detalle_pedido
...
SELECT p.codigo_producto, p.nombre, SUM(dp.cantidad) 
...
WHERE jefe.nombre = 'Alberto' 
AND jefe.apellido1 = 'Soria';
...
NOT IN (SELECT DISTINCT codigo_producto FROM detalle_pedido);
...
NOT IN (SELECT codigo_empleado_rep_ventas FROM cliente WHERE codigo_empleado_rep_ventas IS NOT NULL);
