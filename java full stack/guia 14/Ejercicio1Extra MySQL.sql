...
select e.puntos_por_partido, j.nombre, e.temporada 
from estadisticas e 
...
SELECT e.Nombre 
AS Nombre_equipo, COUNT(j.codigo) 
AS Numero_jugadores 
FROM equipos e 
INNER JOIN jugadores j 
ON e.Nombre = j.Nombre_equipo 
...
SELECT j.Nombre 
AS Nombre_jugador, SUM(e.Puntos_por_partido) 
AS Total_puntos 
FROM jugadores j 
INNER JOIN estadisticas e 
ON j.codigo = e.jugador 
GROUP BY j.codigo, j.Nombre 
HAVING SUM(e.Puntos_por_partido) = ( SELECT MAX(total_puntos) 
FROM (SELECT jugador, SUM(Puntos_por_partido) 
AS total_puntos 
FROM estadisticas 
GROUP BY jugador ) AS subquery);

-- 13. Mostrar el nombre del equipo, conferencia y división del jugador más alto de la NBA.

SELECT e.Nombre 
AS Nombre_equipo, e.Conferencia, e.Division 
FROM jugadores j 
INNER JOIN equipos e 
...
SELECT p.equipo_local, p.equipo_visitante, ABS(p.puntos_local - p.puntos_visitante) 
AS diferencia 
FROM partidos p 
WHERE ABS(p.puntos_local - p.puntos_visitante) = (SELECT MAX(ABS(puntos_local - puntos_visitante))FROM partidos);
...
CASE WHEN puntos_local > puntos_visitante 
THEN equipo_local 
